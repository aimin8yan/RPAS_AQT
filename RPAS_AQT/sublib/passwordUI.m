function password = passwordUI(uitype, title, varargin)
  ...
  if ischar(uitype) 
    val=validatestring(uitype, {'Query', 'Create'});
    uiMode=val;
  else
    error('The first input should be string\n');
  end
  if ischar(title) 
    dialogTitle=title;
  else
    error('The second input should be string\n');
  end
  ...
  for id = 1:numel(varargin)
    if ischar(varargin{id})
        val = validatestring(varargin{id}, ...
            {'Any', 'AlphaOnly', 'NumOnly', 'AlphaNum', ...
            'AlphaNumSpecial'});
        switch val
  ...
  while num_try<max_try
    ...
    if 1
      fh=uifigure('Position',[ct(1)-w/2, ct(2)-h/2, w, h], ...
                  'Name', dialogTitle, ...
                 'CloseRequestFcn', @closeFcn, ...
                 'KeyPressFcn', @passwordKeyPressFcn);
      th = uieditfield('Parent', fh, ...
...
%--------------------------------------------------------------------------
    function passwordKeyPressFcn(obj, edata)
        ...
        th.Value=[repmat('*', 1, length(password)), '_'];
    end
